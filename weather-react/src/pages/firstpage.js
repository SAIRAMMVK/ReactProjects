    ...
    Btnclick() {
        fetch('http://api.openweathermap.org/data/2.5/forecast?q=' + this.state.cityName + '&appid=d14c83dc1f84f75137ecb3c8a6c78fde').
            ...
            then(data => {
               ...
               const listOfDates = data.list.map((ele) => moment(ele.dt * 1000).format('dddd'));
                console.log(listOfDates);
                const listOfTemp = data.list.map(ele => Math.round(ele.main.temp - 270));
                ...
                console.log("in states"+this.state.chartdata.dateslist)
                console.log("in states"+this.state.chartdata.templist)
    ...
    render() {
        return (
            <div>
                ...
                <Box headInfo={this.state.heads} bannerInfo={this.state.tempcontents} chartInfo={this.state.chartdata}  forecast={this.state.forecastInfo} />
            </div>
        );
    }
}